...
import { getAllApps, deleteAppByAdmin, updateAppByAdmin } from '@/api/appController'
  ...
  try {
    ...
    if (res.data.data) {
      ...
      const processedRecords = records.map(record => {
        // 处理创建时间
        if (record.createTime && Array.isArray(record.createTime)) {
          const date = parseDateTimeArray(record.createTime as unknown as number[])
          record.createTime = date ? formatDateTime(date) : '未知时间'
        }

        // 处理部署时间
        if (record.deployedTime && Array.isArray(record.deployedTime)) {
          const date = parseDateTimeArray(record.deployedTime as unknown as number[])
          record.deployedTime = date ? formatDateTime(date) : null
        }

        // 处理更新时间（如果需要显示）
        if (record.updateTime && Array.isArray(record.updateTime)) {
          const date = parseDateTimeArray(record.updateTime as unknown as number[])
          record.updateTime = date ? formatDateTime(date) : '未知时间'
        }

        return record
      })

      data.value = processedRecords
