    ...
    <div class="container">
      ...
      <div class="input-section">
        ...
        <div class="input-actions">
          <a-button type="primary" size="large" @click="createmyApp" :loading="creating">
            <template #icon>
              <span>↑</span>
            </template>
          </a-button>
...
import { createApp, getMyApps, getFeaturedApps } from '@/api/appController'
...
const createmyApp = async () => {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      // 添加调试信息
      // console.log('原始应用ID:', res.data.data)
      // console.log('应用ID类型:', typeof res.data.data)

      // 检查是否为大整数
      if (typeof res.data.data === 'number' &&
        (res.data.data > Number.MAX_SAFE_INTEGER || res.data.data < Number.MIN_SAFE_INTEGER)) {
        // console.warn('警告：应用ID可能因JavaScript精度问题而失真')
      }

      // 跳转到对话页面，确保ID是字符串类型
      const appId = String(res.data.data)
      // console.log('转换后的应用ID:', appId, '类型:', typeof appId)
